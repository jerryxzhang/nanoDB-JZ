...
public class IndexUpdater implements RowEventListener {
        ...
        for (ColumnRefs indexDef : schema.getIndexes().values()) {
            ...
            try {
                ...
                if (constraintType != null && constraintType.isUnique()) {
                    ...
                    idxTup = IndexUtils.makeSearchKeyValue(indexDef, ptup,
                ...
                logger.debug("Adding tuple " + idxTup + " to index " +
                    indexDef.getIndexName());
    ...
    private void removeRowFromIndexes(TableInfo tblFileInfo, PageTuple ptup) {
        ...
        TableSchema schema = tblFileInfo.getSchema();
        for (ColumnRefs indexDef : schema.getIndexes().values()) {
            TupleLiteral idxTup = IndexUtils.makeSearchKeyValue(indexDef, ptup,
                /* findExactTuple */ true);

            logger.debug("Removing tuple " + idxTup + " from index " +
                indexDef.getIndexName());
            ...
            try {
                ...
                if (idxPageTup == null) {
                    throw new IllegalStateException("Can't find tuple in " +
                        "index corresponding to table's tuple.");
                }
