...
public abstract class PageTuple implements Tuple {
    ...
    public PageTuple(DBPage dbPage, int pageOffset, Schema schema) {
        ...
        pin();  // Pin ourselves immediately so that we don't lose the DBPage.
    ...
    public void unpin() {
        ...
        if (pinCount == 0) {
            // Invalidate the tuple by clearing out our DBPage references etc.,
            // since we can no longer trust the data page's contents.
